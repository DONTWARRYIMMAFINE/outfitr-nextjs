...
const OrderItem: FC<OrderItemProps> = ({ orderItem, t }) => {
  return (
    <Box display={"flex"} gap={2}>
      <Box position={"relative"} overflow={"hidden"} width={180} height={225} borderRadius={0.25}>
        <Image priority src={orderItem.productVariant.product.media[0]?.url} alt={"Product variant thumbnail"} style={{ objectFit: "cover" }} fill />
      </Box>
      <Box width={"100%"}>
        <Grid item xs={12}>

...
const CartItem: FC<CartItemProps> = ({ cartItem, onAddClick, onRemoveClick }) => {
  ...
  return (
    <Grid container padding={3} justifyContent={"center"} alignItems={"center"}>
      <Grid container item xs={7}>
        ...
        <Grid display={"flex"} flexDirection={"column"} item xs={10} gap={2} paddingX={3}>
          <Text variant={"h4"} component={"h4"}>{cartItem.productTitle}</Text>
          <Box display={"flex"} flexDirection={"column"}>
            <Box>
              <Text variant={"p"}>Size: </Text>
